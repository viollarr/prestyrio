...
if($rows == 0){
	...
	if($result[0] == 00){	  	
	  ...
	  elseif($result[0] == 01){
		...
		$referencia = trim(substr($linha,820,250));
		...
		if($rows_user == 0){
			if($notaFiscalSerie != "000000 / "){
				$select_user_termporario = "SELECT * FROM clientes_assistencia WHERE (orcamento = \"$orcamento\" AND cod_loja = \"$loja\" AND data_faturamento = \"$data_final\" AND nome_cliente = \"$nome\" AND cep_cliente = \"$cep\")";
