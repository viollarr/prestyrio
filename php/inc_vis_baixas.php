...
$select = "SELECT c.prioridade, c.tipo, c.nome_cliente, c.orcamento, d.data_saida_montador, d.data_entrega_montador, d.data_final, o.porcentagem, o.n_montagem, o.comentario, o.status, o.id_montador, o.id_montagem, o.wa_tabela_preco FROM ordem_montagem o, datas d, clientes c WHERE o.id_montagem = '".$_GET['id_montagem']."' AND (c.n_montagem = o.n_montagem AND c.n_montagem = d.n_montagens AND o.n_montagem = d.n_montagens )";
...
if($rows_pre > 0){
	...
	$data_pre = new DateTime($result_pre['data_final']);  
	$data_pre = $data_pre->format('d/m/Y');

	
	$data_montagem = "value='$data_pre'";
	$comentario = $result_pre['comentario'];
	$texto = 'Pré-baixa já realizada conforme datas e comentários abaixo.';
}
else{
...
if($x["wa_tabela_preco"] == 2){
	$tabela_alta2 = ' checked="checked"';
}
else{
	$tabela_alta = ' checked="checked"';
}
...
<table width="550" border="0" align="center" cellpadding="2" cellspacing="2" class="cor_tr texto">
          ...
          <tr>
          	...
          	<td colspan="2" align="center"><strong>PARA VALIDAR A NOTA FAVOR PREENCHA OS DADOS ABAIXO</strong><br /><br /><?=$texto?></td>
          ...
          <tr>
          	...
          	<td colspan="2">Tipo de Tabela usada: <input type="radio" name="tabela"<?=$tabela_alta?> value="1" /> <span style="color:#F00;">TABELA NORMAL</span>    <input type="radio" name="tabela"<?=$tabela_alta2?> value="2" /> <span style="color:#F00;">TABELA ALTA (somente WA)</span></td>
          ...
          <tr>
          	...
          	<td colspan="2">Data da Montagem: <input type="text" name="data_final" maxlength="10" size="15" <?=$data_montagem?> class="data datepicker" /></td>
