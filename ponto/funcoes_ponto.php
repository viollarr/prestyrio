...
function conecta_bd()
{
 global $dbHost,$dbUser,$dbPass, $dbName, $dbConexao, $dbSelect;
 $dbConexao = mysql_connect($dbHost,$dbUser,$dbPass);
 if(!$dbConexao)
 {
  print "Erro conectando ao BD:\n<br>" . mysql_errno($dbConexao) . "\n<br>" . mysql_error($dbConexao) . "\n<br>";
  echo "<span style=\"font-family: verdana; font-size: 12px; color: #FF0000;\"><b>Erro</b> na conexXo com o banco...</span><br>".mysql_error();
  exit;
 }
 $dbSelect = mysql_select_db($dbName);
 if(!$dbSelect)
 {
  print "Erro selecionando o BD:\n<br>" . mysql_errno($dbConexao) . "\n<br>" . mysql_error($dbConexao) . "\n<br>";
  echo "<span style=\"font-family: verdana; font-size: 12px; color: #FF0000;\"><b>Erro</b> na seleXXo do banco...</span><br>".mysql_error();
  exit;
 }
}
#########
...
function get_dia_especial($DIA_I,$DIA_P)
{
 ...
 global $FERIADOS_DIAS2, $DIAS_MEIO_EXPEDIENTE2;
 ...
 if(isset($FERIADOS_DIAS2[$DIA_I]))
 {
  ##print "Feriado"; exit();
  return(true); 
 }
 ###if(isset($DIAS_MEIO_EXPEDIENTE[$DIA_P]))
 if(isset($DIAS_MEIO_EXPEDIENTE2[$DIA_I]))
 {
