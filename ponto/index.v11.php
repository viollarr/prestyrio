...
$DIA_ESPECIAL=false;	#Marque se e' sabado, domingo ou feriado.
...
$DIA_ESPECIAL = get_dia_especial($HOJE_I,$HOJE_P);	#Feriado, sabado, domingo, meio-expediente?
#print "DIA_ESPECIAL: $DIA_ESPECIAL"; exit();
...
{
  #Se e' por POST encerre a sessao de admin para evitar na mesma maq. alguem logado e puder bater o ponto a qq hora.
  session_start();
  session_unset();
  session_destroy();
  ##unset($_SESSION); Erro: Do NOT unset the whole $_SESSION with unset($_SESSION) as this will disable the registering of session variables through the $_SESSION superglobal.
  $_SESSION = array();
}
	...
	else
	{
		...
		*/
		...
		##print_r($arrDados);
                ...
                ##print "host_name: $host_name"; exit();
                $QUEM_BATEU=$nmCracha;		#'fu_matricula' e 'fu_num_cracha' sao diferentes. nmCracha='fu_num_cracha', 'fu_nome'
                $QUEM_BATEU .= ": " . $arrDados['fu_nome'];
                ##print "QUEM_BATEU: $QUEM_BATEU"; exit();
                                
                if(preg_match("/admin$|superadm$/i",$_SESSION['adm_grupo']) )
                {
                  #Se for batido pelo adm devidadmente logado aceite o ponto e login login=$_SESSION[login]  ....
                  $QUEM_BATEU .= " <- " . $_SESSION['adm_login'] .  ": " . $_SESSION['adm_nome'];	#adm_login adm_nome
                }
                else if($nr_batidas_do_dia < 1 and !$DIA_ESPECIAL)	#E' a primeira batiada = entrada.
                {
                 #Testa o horario
		...
		$QUEM_BATEU=addslashes($QUEM_BATEU);
		$queryP	= "INSERT INTO ponto(fu_id,po_data,po_semana,po_ip, po_ip_reverso,po_quem_bateu, po_valido ) 
			VALUES (".$arrDados['fu_id'].",'$agora_date_time','$nmSemana','$ip',
			'$host_name','$QUEM_BATEU','$VALIDO')";
		##print "queryP: $queryP";
		##$sqlPonto = mysql_query($queryP);
		##print "queryP: $queryP"; exit();
...
if(!$resultado)
{
 print "Nr. Cracha n&atilde;o encontrado!";	# resultado: $resultado"; exit();
 exit();
}
